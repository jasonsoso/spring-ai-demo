...
- MCP Server 暴露地址：`http://localhost:8080`（SSE 模式）
...
```sql
CREATE DATABASE spring_ai_agent CHARACTER SET utf8mb4;
```
...
```bash
./mvnw clean package
java -jar target/spring-ai-demo-*.jar
```
...
| 地址 | 说明 |
|------|------|
| `http://localhost:8080` | 前端演示界面（多 Tab） |
...
# ===== MySQL =====
spring.datasource.url=jdbc:mysql://127.0.0.1:3306/spring_ai_agent
...
详细接口请访问 `http://localhost:8080/swagger-ui.html`，以下为接口速查表：
...
sequenceDiagram
    ...
    participant McpServer SSE :8080
    participant WeatherTool
    participant AttractionTool
    participant DeepSeek

    Note over McpServer SSE :8080: 启动时 McpServerConfig 注册工具
    McpServer SSE :8080->>WeatherTool: 注册 getWeather
    McpServer SSE :8080->>AttractionTool: 注册 recommendAttractions

    Note over McpClientInitializer: ApplicationReadyEvent @Order(1)
    McpClientInitializer->>McpServer SSE :8080: SSE 连接 http://localhost:8080
    McpServer SSE :8080-->>McpClientInitializer: 返回工具列表
    ...
    McpChatController->>McpServer SSE :8080: 转发工具调用
    McpServer SSE :8080->>WeatherTool: 执行
    WeatherTool-->>McpServer SSE :8080: 结果
    McpServer SSE :8080-->>McpChatController: 工具结果
...
```
demo/
├── src/main/java/com/jason/demo/demo/
│   ├── DemoApplication.java          # 启动类（排除 Milvus 自动配置）
│   ├── config/                       # 配置类
...
MCP Client 连接本机 MCP Server（`http://localhost:8080`），需确保应用已完全启动再发起请求。初始化顺序由 `@Order` 控制，若出现 NPE 可在请求前稍等片刻。
