...
**`controller` 包共 17 个 Controller**（另含 `mcp.client.controller.McpChatController`），与下表一一对应；功能模块章节按相同顺序归档。
...
| 工具库 | Lombok | — |
| 前端 | 原生 HTML/CSS/JS（单页 17 Tab，按模块拆分静态资源，零构建） | — |
...
| 其他 Tab | Embedding、RAG、Agent 记忆（内存/MySQL/自主记忆）、MCP 等 |
...
```
static/
├── index.html              # 骨架：Tab 导航 + 17 个功能面板 HTML
├── css/
...
│           └── tabs/                 # 15 个 Tab 逻辑文件
├── src/test/java/.../
│   └── SessionMemoryTripAgentServiceTest.java  # Session 记忆单元测试
├── scripts/
