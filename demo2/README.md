...
| Agent 工具库 | spring-ai-agent-utils | 0.10.0 |
...
| `AskUserAgentService` | Agent 编排、异步执行 |
| `WebQuestionHandler` | 实现 `QuestionHandler`，推送问题并 `CompletableFuture` 阻塞等待 |
| `AskUserSessionStore` | 内存 Session、SSE 事件缓冲与 flush |
...
├── src/main/resources/
│   ├── application.properties        # 主配置（含 Micrometer/OTel，OTLP 默认关闭）
...
- [Spring AI Agent Utils（AskUserQuestionTool）](https://github.com/spring-ai-community/spring-ai-agent-utils)
