...
在 `demo2` 中新增 **「☕ 瑞幸 MCP 点单」** Tab，通过 Spring AI MCP Client 对接 [瑞幸咖啡 AI 开放平台](https://open.lkcoffee.com/mcp) 远程 MCP Server，结合 [官方 My Coffee Skill](https://open.lkcoffee.com/skill) 编排点单流程，以及 [高德地图 MCP Server](https://lbs.amap.com/api/mcp-server) 完成地址地理编码，以 **SSE 对话式** UI 演示完整点单链路：查门店 → 选品 → 预览 → 用户确认 → 下单/支付二维码。
...
| 交互编排 | **官方 My Coffee Skill v0.8.2**（`SKILL.md` 内嵌为 System Prompt，**不用 SkillsTool**） |
...
Skill 包来源（实现时 vendoring 到 classpath）：

```
https://unpkg.luckincoffeecdn.com/@luckin/my-coffee-skill@latest/dist/my-coffee-skill.zip
```

当前版本：`0.8.2`（`instruction-only`，License CC BY-ND 4.0，原样引用不修改 Skill 正文）。

### 1.4 依赖

- Spring AI 2.0.0（已有 `spring-ai-starter-mcp-client`）
- 官方 My Coffee Skill `SKILL.md`（vendoring 到 `classpath:/.claude/skills/my-coffee/`）
- 现有本地 MCP Server + Client（**不修改**，与新 Tab 隔离）
- 无需新增 Maven 依赖；**不引入 SkillsTool**（Dedicated Tab 直接注入 Skill 内容，避免语义匹配额外开销）
...
│  System Prompt = 官方 SKILL.md + 项目覆盖规则                │
...
| 层 | 组件 | 职责 |
|----|------|------|
| 编排层 | My Coffee `SKILL.md` | 点单流程、确认规则、优惠券透传、支付展示 |
| 能力层 | 瑞幸 MCP Server | 8 个下单工具 |
| 能力层 | 高德 MCP Server | 地址 ↔ 经纬度 |
...
| `LkCoffeeSkillLoader` | `service` 或 `config` | 启动时读取 `SKILL.md`，拼接覆盖规则生成 System Prompt |
| `LkCoffeeAgentService` | `service` | SSE 流式对话、结构化事件解析 |
...
# 官方 My Coffee Skill（内嵌为 System Prompt，非 SkillsTool 扫描）
...
### 4.1 System Prompt：官方 Skill + 项目覆盖规则

**方案：内嵌 SKILL.md 为 System Prompt（不用 SkillsTool）**

Dedicated Tab 每次请求上下文固定为「瑞幸点单」，无需语义匹配加载 Skill。启动时由 `LkCoffeeSkillLoader` 读取 `agent.lkcoffee.skill` 指向的 `SKILL.md` 全文，再追加 **项目覆盖规则**（优先级高于 Skill 默认行为）。

**Skill 提供的核心价值（保留原样）**

- 完整点单流程：`确认门店 → 确认商品与意图 → previewOrder → createOrder`
- 优惠券 `couponCodeList` 原样透传
- 仅展示 `payOrderQrCodeUrl`，不展示 `payOrderUrl`
- 不支持外送，仅到店自取
- 工具调用过程对用户不可见（不暴露工具名、原始 JSON）
- 缺参时单点追问，避免并发追问

**项目覆盖规则（追加在 SKILL.md 之后，优先级更高）**
...
4. 定位：
   - 优先使用前端请求附带的 longitude/latitude。
...
| 维度 | Skills Tab | 瑞幸 Tab |
|------|-----------|----------|
| Skill 加载 | `SkillsTool` 语义匹配 | 启动时读入 `SKILL.md` 固定注入 |
| 工具 | Shell/FileSystem/Glob/Grep | 瑞幸 MCP + 高德 MCP |
| 场景 | 通用多 Skill | Dedicated 点单 |

### 4.2 ChatClient 构建

```java
chatClientBuilder.clone()
    .defaultSystem(lkCoffeeSkillLoader.buildSystemPrompt())  // SKILL.md + 覆盖规则
...
```
┌─ Header + MCP + Skill 三层架构说明 ─────────────┐
│  用户 → Agent（My Coffee Skill 编排）→ 瑞幸/高德 MCP │
├─ 设置区（可折叠）────────────────────────────────┤
...
| 操作 | 路径 |
|------|------|
| 新增 | `resources/.claude/skills/my-coffee/SKILL.md`（从官方 zip vendoring，v0.8.2） |
| 新增 | `resources/.claude/skills/my-coffee/manifest.json`（可选，标注版本） |
| 新增 | `config/LkCoffeeAgentConfig.java` |
| 新增 | `service/LkCoffeeSkillLoader.java`（或置于 config） |
...
| 10 | Skill 加载 | 启动日志确认 `SKILL.md` 已读入；Agent 遵循「仅展示二维码、不支持外送」等 Skill 规则 |
