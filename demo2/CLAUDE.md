...
| `service.common` | 状态枚举 `*Status`、错误码 `*ErrorCode`（实现 `ErrorCode`） |
...
| 错误码 | `{Module}ErrorCode` | `OrderErrorCode` |
...
- 业务失败抛 `BusinessException({Module}ErrorCode.xxx)`，**不要**自定义 `*DomainException`
- 全局 `Demo2GlobalExceptionHandler` 只捕获 `BusinessException`，HTTP 始终 200，前端以 `code === 0` 判定成功
- Controller 返回 `JsonResult<T>`，用 `JsonResults.ok(...)` 包装
- 错误码分段：`0` 成功 / `1xxxx` 通用 / `2xxxx` 会员 / `3xxxx` 订单 / `4xxxx` 商品（预留）
